  ...
  await withServer(
    ...
    async (baseUrl) => {
      const resp = await nodeHttpTransport({
        method: "GET",
        url: `${baseUrl}/genesisws/rest/2020/helloworld/whoami`,
      });
      assert.equal(resp.status, 200);
      assert.equal(resp.headers["content-type"], "application/json");
      assert.deepEqual(JSON.parse(resp.body.toString("utf8")), {
        path: "/genesisws/rest/2020/helloworld/whoami",
      });
    },
  );
});

test("rejects an unsupported protocol with RegionalstatistikNetworkError", async () => {
  ...
  await withServer(
    ...
    async (baseUrl) => {
      ...
      await assert.rejects(
        ...
        (err) => err instanceof RegionalstatistikNetworkError && /timed out after 300ms/.test(err.message),
      );
